// 开挖分析，底座图层
export class CutBasemapTileLayer {
  static async create (properties) {
    ...
    const {
      ...
      geometryEngine,
      Extent
    } = await arcgis.esriLoader('BaseTileLayer', 'geometryEngine', 'Extent')
    const CutBasemapTileLayerClass = BaseTileLayer.createSubclass({
      constructor: function (properties) {
        this.cutGeometries = properties.cutGeometries || []
        this.view = properties.view
        this.baseColor = properties.baseColor || '#AAAAAA'
      },
      ...
      fetchTile: async function (level, row, col, options) {
        ...
        this.cutGeometries.forEach((geometry) => {
          const intersection = geometryEngine.intersect(extent, geometry)
          if (intersection) {
            // const buffer = geometryEngine.buffer(intersection, 5, 'meters')
            context.beginPath()
    ...
    return new CutBasemapTileLayerClass(properties)
  }
}
